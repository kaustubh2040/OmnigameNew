    ...
    <div className="max-w-6xl mx-auto p-6">
      <header className="flex items-center justify-between mb-12">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">OmniPlay</h1>
            <p className="text-zinc-500 text-sm">Welcome back, {profile?.username}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-xl flex flex-col gap-1 min-w-[180px]">
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-zinc-500 uppercase font-black tracking-wider">Level {profile?.level}</p>
              <p className="text-[10px] text-zinc-400 font-mono">{profile?.xp}{nextLevelXp ? ` / ${nextLevelXp}` : ''} XP</p>
            </div>
          ...
          <button 
            onClick={signOut}
            className="p-2.5 bg-zinc-900 border border-zinc-800 rounded-xl hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-500 transition-all"
          >
        ...
        <div className="lg:col-span-2 space-y-8">
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <button 
                ...
                disabled={loading}
                className="group p-6 bg-zinc-900 border border-zinc-800 rounded-2xl hover:border-emerald-500/50 transition-all text-left relative overflow-hidden"
              >
                <div className="relative z-10">
                  <h3 className="text-xl font-bold mb-1">Tic Tac Toe</h3>
                  <p className="text-zinc-500 text-sm">Classic 3x3 turn-based strategy</p>
                </div>
              ...
              >
                <div className="relative z-10">
                  <h3 className="text-xl font-bold mb-1">Rock Paper Scissors</h3>
                  <p className="text-zinc-500 text-sm">Simultaneous move battle</p>
                </div>
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <Gamepad2 className="w-16 h-16" />
                </div>
          ...
          <section>
            ...
            <form onSubmit={joinRoom} className="flex gap-3">
              <input 
                type="text"
                placeholder="ENTER CODE (E.G. G7X2P9)"
                value={roomCode}
                onChange={(e) => setRoomCode(e.target.value.toUpperCase())}
                className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-mono tracking-widest"
              />
              <button 
                type="submit"
                disabled={loading || !roomCode}
                className="bg-emerald-500 hover:bg-emerald-600 text-zinc-950 px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-all disabled:opacity-50"
              >
          ...
          <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              {leaderboard.map((entry, i) => (
                <div key={entry.user_id} className="flex items-center justify-between p-3 bg-zinc-950 rounded-xl border border-zinc-800/50">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium truncate max-w-[100px]">{entry.users?.username}</span>
                  </div>
